...
export async function generateAssessmentReport(data: AssessmentReportData): Promise<Buffer> {
  ...
  // ═══════════════════════════════════════════════════════════════════════
  // SECTION 7B: SECTOR WEIGHTING + X-RAY STRUCTURAL FINDINGS
  // ═══════════════════════════════════════════════════════════════════════

  if (scoringResult.sectorWeighting && typeof scoringResult.baselineOverallScore === 'number'
      && Math.abs(scoringResult.overallScore - scoringResult.baselineOverallScore) >= 1) {
    children.push(headingParagraph('Sector Weighting', HeadingLevel.HEADING_2));
    children.push(bodyParagraph(
      `Pillar weights were re-balanced for the ${scoringResult.sectorWeighting.sector} sector and renormalised to 1.0. ` +
      `Unweighted baseline: ${Math.round(scoringResult.baselineOverallScore)}% → sector-weighted overall: ${Math.round(scoringResult.overallScore)}%. ` +
      scoringResult.sectorWeighting.rationale,
      { color: COLOR_TEXT_SECONDARY, size: 20, spacing: { before: 40, after: 120 } },
    ));
  }

  if (scoringResult.xRayFindings && scoringResult.xRayFindings.length > 0) {
    children.push(headingParagraph('Structural Findings (X-Ray)', HeadingLevel.HEADING_2));
    children.push(bodyParagraph(
      'These findings are detected deterministically from how your responses combine across pillars — two organisations with the same overall score can carry entirely different structural risks. Each finding cites the question-level evidence that triggered it. They are the primary input to the recommendations that follow.',
      { color: COLOR_TEXT_SECONDARY, size: 20, spacing: { before: 40, after: 120 } },
    ));
    const sevColor = (sev: string): string =>
      sev === 'critical' ? COLOR_RED : sev === 'high' ? COLOR_AMBER : sev === 'medium' ? COLOR_BLUE : COLOR_TEXT_SECONDARY;
    const xrHeader = new TableRow({
      children: [
        makeHeaderCell('ID', 900),
        makeHeaderCell('Severity', 1200),
        makeHeaderCell('Finding', 3600),
        makeHeaderCell('Business impact', 3000),
        makeHeaderCell('Recommended move', 3000),
      ],
      tableHeader: true,
    });
    const xrRows = scoringResult.xRayFindings.map((f, idx) => new TableRow({
      children: [
        makeCell(f.id, { bold: true, color: sevColor(f.severity), width: 900, shading: idx % 2 === 1 ? BG_ROW_ALT : undefined }),
        makeCell(f.severity.toUpperCase(), { bold: true, color: sevColor(f.severity), width: 1200, shading: idx % 2 === 1 ? BG_ROW_ALT : undefined }),
        makeCell(`${f.title} — ${f.headline}`, { size: 18, width: 3600, shading: idx % 2 === 1 ? BG_ROW_ALT : undefined }),
        makeCell(f.businessImpact, { size: 18, color: COLOR_TEXT_SECONDARY, width: 3000, shading: idx % 2 === 1 ? BG_ROW_ALT : undefined }),
        makeCell(f.recommendation, { size: 18, width: 3000, shading: idx % 2 === 1 ? BG_ROW_ALT : undefined }),
      ],
      cantSplit: true,
    }));
    children.push(new Table({
      rows: [xrHeader, ...xrRows],
      width: { size: 100, type: WidthType.PERCENTAGE },
      borders: { top: thinBorder(BORDER_MEDIUM), bottom: thinBorder(BORDER_MEDIUM), left: thinBorder(BORDER_MEDIUM), right: thinBorder(BORDER_MEDIUM), insideHorizontal: thinBorder(BORDER_LIGHT), insideVertical: thinBorder(BORDER_LIGHT) },
      layout: TableLayoutType.FIXED,
    }));
  ...
  // Steps generated by the LLM follow "Action | Owner: … | Timeline: … |
  // Success metric: …". When every step parses, render the consultancy-style
  // action table; otherwise fall back to the numbered list.
  const parsedSteps = insights.nextSteps.map((step) => {
    const parts = step.split(' | ');
    const get = (prefix: string) => parts.find((p) => p.trim().toLowerCase().startsWith(prefix))?.split(':').slice(1).join(':').trim();
    const owner = get('owner');
    const timeline = get('timeline');
    const metric = get('success metric');
    return owner && timeline ? { action: parts[0]?.trim() ?? step, owner, timeline, metric: metric ?? '—' } : null;
  });
  if (parsedSteps.length > 0 && parsedSteps.every((x) => x !== null)) {
    const recHeader = new TableRow({
      children: [
        makeHeaderCell('#', 500),
        makeHeaderCell('Action', 5200),
        makeHeaderCell('Owner', 2200),
        makeHeaderCell('Timeline', 1700),
        makeHeaderCell('Success metric', 2800),
      ],
      tableHeader: true,
    });
    const recRows = parsedSteps.map((r, idx) => new TableRow({
      children: [
        makeCell(String(idx + 1), { bold: true, color: COLOR_BLUE, alignment: AlignmentType.CENTER, width: 500, shading: idx % 2 === 1 ? BG_ROW_ALT : undefined }),
        makeCell(r!.action, { size: 18, width: 5200, shading: idx % 2 === 1 ? BG_ROW_ALT : undefined }),
        makeCell(r!.owner, { size: 18, color: COLOR_TEXT_SECONDARY, width: 2200, shading: idx % 2 === 1 ? BG_ROW_ALT : undefined }),
        makeCell(r!.timeline, { size: 18, color: COLOR_AMBER, width: 1700, shading: idx % 2 === 1 ? BG_ROW_ALT : undefined }),
        makeCell(r!.metric, { size: 18, color: COLOR_TEXT_SECONDARY, width: 2800, shading: idx % 2 === 1 ? BG_ROW_ALT : undefined }),
      ],
      cantSplit: true,
    }));
    children.push(new Table({
      rows: [recHeader, ...recRows],
      width: { size: 100, type: WidthType.PERCENTAGE },
      borders: { top: thinBorder(BORDER_MEDIUM), bottom: thinBorder(BORDER_MEDIUM), left: thinBorder(BORDER_MEDIUM), right: thinBorder(BORDER_MEDIUM), insideHorizontal: thinBorder(BORDER_LIGHT), insideVertical: thinBorder(BORDER_LIGHT) },
      layout: TableLayoutType.FIXED,
    }));
