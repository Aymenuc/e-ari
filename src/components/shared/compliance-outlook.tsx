      ...
      <CardHeader className="pb-2">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          {!noLinkedUseCases && baselineHref ? (
            <Link
              href={baselineHref}
              className="font-heading text-xs text-slate-300 underline-offset-4 hover:underline"
            >
              Manage use cases
            </Link>
          ) : null}
          ...
          <>
            ...
            <div>
              <div className="mb-1 flex items-center justify-between text-[10px] font-mono uppercase tracking-wide text-muted-foreground">
                <span>Coverage</span>
                <span className="text-slate-300">{outlook.coveragePct}%</span>
              </div>
...
function Metric({
  ...
  return (
    <div className="rounded-lg border border-white/[0.06] bg-navy-900/40 p-3">
      <Icon className="mb-2 h-4 w-4 text-slate-300" aria-hidden />
